...
# IMPORTANT: This must match the "Redirect URI" in your App settings.
...
def cc_authorize():
    ...
    auth_url = (
        f"https://identity.constantcontact.com/oauth2/aus1lm3ry9mF7x2Ja0h8/v1/authorize?" # <-- Use THIS base URL
        f"response_type=code&client_id={CONSTANT_CONTACT_CLIENT_ID}"
        f"&redirect_uri={CC_REDIRECT_URI}&scope=contact_data&state={state}"
    )
    return redirect(auth_url)
...
def callback():
    ...
    try:
        ...
        return f"Error fetching Mailchimp metadata: {e}", 500
    except KeyError:
        return "Error: 'dc' (server prefix) not found in Mailchimp metadata response.", 500
